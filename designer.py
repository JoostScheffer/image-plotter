...
class QPaletteButton(QtWidgets.QPushButton):
    def __init__(self, rgb_color):
        ...
        self.setStyleSheet("border :1px solid ;background-color: %s;" % self.color)
...
class FloatingPreview(QtWidgets.QWidget):
    def __init__(self, input_img: ImgFile, opacity: int):
        super().__init__()
    ...
    def __update_opacity(self):
        ...
        painter.drawPixmap(QtCore.QPoint(), self.m_original_input_image.qpix_scaled)
        painter.end()
...
class DrawingWorker(QtCore.QThread):
    ...
    def start_drawing(
        ...
        self.qpix_input_image: ImgFile = output_img
            ...
            for j in self.que[i]:
                ...
                if not colors_placed % 40:
                    ...
                    if curr_precent > prev_percent:
                        ...
                        # sleep(0.2)
                if keyboard.is_pressed("escape") or keyboard.is_pressed("shift"):
                    self.progress_signal.emit(0)
                    return
...
class FloydWorker(QtCore.QThread):
    ...
    def __init__(self):
        ...
        # self.qpix_input_image: ImgFile = None
        ...
        self.ordered_dither_functions = {
            "Yliluoma": hitherdither.ordered.yliluoma.yliluomas_1_ordered_dithering,
            "Bayer matrix": hitherdither.ordered.bayer.bayer_dithering,
            "Cluster dot matrix": hitherdither.ordered.cluster.cluster_dot_dithering,
        }
    ...
    def run(self):
        ...
        intermediate_filename = ".".join(intermediate_filename_parts[:-1]) + ".png"
...
class Ui_Image_drawer(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.DEFAULT_INPUT_IMG = ImgFile("./sources/eend.png", self.DEFAULT_IMG_MENU_SIZE)
        self.DEFAULT_OUTPUT_IMG = ImgFile("./sources/eend_out.png", self.DEFAULT_IMG_MENU_SIZE)
        ...
        self.drawing_speed = self.AVAILABLE_DRAWING_SPEEDS[self.DEFAULT_DRAWING_SPEED_IDX]
        # waiting speeds in seconds
        self.AVAILABLE_WAITING_SPEEDS = [0.000001, 0.15, 1, 4]
        self.DEFAULT_WAITING_SPEED_IDX = 1
        self.waiting_speed = self.AVAILABLE_WAITING_SPEEDS[self.DEFAULT_WAITING_SPEED_IDX]
        ...
        self.floating_image_preview_window = FloatingPreview(
            ImgFile(self.output_img.filename, self.output_img_size[0]),
    ...
    def UI_vertical_layout(self):
        ...
        self.veticalLayout_outer.setSizeConstraint(QtWidgets.QLayout.SetMinimumSize)
        ...
        self.veticalLayout_inner.setSizeConstraint(QtWidgets.QLayout.SetMinimumSize)
    ...
    def UI_group_menu_img(self):
        ...
        self.gridLayout_menu_images = QtWidgets.QGridLayout(self.groupBox_menu_images)

        self.label_input_image = QtWidgets.QLabel(self.groupBox_menu_images)
        self.gridLayout_menu_images.addWidget(self.label_input_image, 0, 0, 1, 1)

        self.label_output_image = QtWidgets.QLabel(self.groupBox_menu_images)
        self.gridLayout_menu_images.addWidget(self.label_output_image, 0, 1, 1, 1)

        self.UI_pixmap_input_image()
        self.gridLayout_menu_images.addWidget(self.label_pixmap_input_image, 1, 0, 1, 1)

        self.UI_pixmap_output_image()
        self.gridLayout_menu_images.addWidget(self.label_pixmap_output_image, 1, 1, 1, 1)
    ...
    def UI_group_color_input(self):
        ...
        self.verticalLayout_color_input = QtWidgets.QVBoxLayout(self.groupBox_color_input)
        ...
        self.checkBox_input_colors.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.gridLayout_color_input.addWidget(self.checkBox_input_colors, 0, 0, 1, 1)
        self.checkBox_input_colors.toggled.connect(self.load_color_from_cursor)

        # spinbox
        self.spinBox_input_colors = QtWidgets.QSpinBox(self.groupBox_color_input)
        self.spinBox_input_colors.setReadOnly(True)
        self.spinBox_input_colors.setButtonSymbols(QtWidgets.QAbstractSpinBox.NoButtons)
        self.spinBox_input_colors.setMaximum(16581375)
        self.gridLayout_color_input.addWidget(self.spinBox_input_colors, 0, 1, 1, 1)
        ...
        # preview
        self.verticalLayout_color_input.addLayout(self.horizontalLayout_colorboxes)
    ...
    def UI_group_output_settings(self):
        ...
        self.verticalLayout_output_settings = QtWidgets.QVBoxLayout(self.groupBox_output_settings)
        ...
        self.verticalLayout_output_settings.addLayout(self.gridLayout_output_settings)
        ...
        self.spinBox_output_resolution_x.setProperty("value", self.output_img_size[0])
        self.spinBox_output_resolution_x.valueChanged.connect(self.update_output_resolution)
        self.gridLayout_output_settings.addWidget(self.spinBox_output_resolution_x, 0, 0, 1, 1)

        # label x
        self.label_output_resolution_x = QtWidgets.QLabel(self.centralwidget)
        self.gridLayout_output_settings.addWidget(self.label_output_resolution_x, 0, 1, 1, 1)
        ...
        self.spinBox_output_resolution_y.setButtonSymbols(QtWidgets.QAbstractSpinBox.NoButtons)
        self.spinBox_output_resolution_y.setMaximum(6000)
        self.spinBox_output_resolution_y.setProperty("value", self.output_img_size[1])
        self.gridLayout_output_settings.addWidget(self.spinBox_output_resolution_y, 1, 0, 1, 1)

        # label y
        self.label_output_resolution_y = QtWidgets.QLabel(self.centralwidget)
        self.gridLayout_output_settings.addWidget(self.label_output_resolution_y, 1, 1, 1, 1)
        ...
        self.comboBox_dither_method.activated[str].connect(self.assign_dither_method)
        # self.gridLayout_output_settings.addWidget(self.comboBox_dither_method,
        #                                           2, 0, 1, 1)
        self.verticalLayout_output_settings.addWidget(self.comboBox_dither_method)
        ...
        #                                           3, 0, 1, 1)
        self.verticalLayout_output_settings.addWidget(self.comboBox_scale_method)

    def UI_group_preview_settings(self):
        self.groupBox_preview_settings = QtWidgets.QGroupBox(self.centralwidget)

        # main vertical layout in group
        # houses label and grid
        self.verticalLayout_preview_settings = QtWidgets.QVBoxLayout(self.groupBox_preview_settings)
        ...
        self.verticalLayout_preview_settings.addWidget(self.pushButton_generate_output)
        ...
        self.verticalLayout_preview_settings.addWidget(self.checkBox_display_image_placing)
        ...
        self.gridLayout_floating_preview.addWidget(self.label_output_size, 0, 0)

        # dial ouput size
        self.dial_output_size = QtWidgets.QDial(self.centralwidget)
        self.dial_output_size.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        ...
        self.gridLayout_floating_preview.addWidget(self.label_output_opacity, 0, 1)

        # dial output opacity
        self.dial_output_opacity = QtWidgets.QDial(self.centralwidget)
        self.dial_output_opacity.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.dial_output_opacity.setMinimum(0)
        self.dial_output_opacity.setMaximum(100)
        self.dial_output_opacity.setProperty("value", self.DEFAULT_PREVIEW_OPACITY)
        self.dial_output_opacity.setTracking(True)
        self.dial_output_opacity.valueChanged.connect(self.opacity_floating_placer)
        self.dial_output_opacity.setOrientation(QtCore.Qt.Horizontal)
        # self.verticalLayout_output_size.addWidget(self.dial_output_size)
        self.gridLayout_floating_preview.addWidget(self.dial_output_opacity, 1, 1)

        self.verticalLayout_preview_settings.addLayout(self.gridLayout_floating_preview)
    ...
    def UI_group_mouse_settings(self):
        ...
        # main vertical layout in group
        # houses label and grid
        self.verticalLayout_mouse_settings = QtWidgets.QVBoxLayout(self.groupBox_mouse_settings)

        # houses set home and set background color
        self.horizontalLayout_mouse_settings = QtWidgets.QHBoxLayout()
        self.verticalLayout_mouse_settings.addLayout(self.horizontalLayout_mouse_settings)

        # checkbox set home
        self.checkBox_set_home = QtWidgets.QCheckBox()
        self.checkBox_set_home.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        ...
        self.horizontalLayout_mouse_settings.addWidget(self.progressBar_home_set)

        # checkbox background color
        self.checkBox_set_background_color = QtWidgets.QCheckBox()
        self.checkBox_set_background_color.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.checkBox_set_background_color.toggled.connect(self.load_color_from_cursor)
        self.horizontalLayout_mouse_settings.addWidget(self.checkBox_set_background_color)

        # colorbox background color
        self.colorBox_background_color = QPaletteButton(self.background_color)
        self.horizontalLayout_mouse_settings.addWidget(self.colorBox_background_color)

        # button start drawing
        self.pushButton_start_drawing = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_start_drawing.setEnabled(True)
        self.pushButton_start_drawing.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.pushButton_start_drawing.clicked.connect(self.draw_output)
        self.verticalLayout_mouse_settings.addWidget(self.pushButton_start_drawing)

        # add drawing speed
        self.UI_vertical_layout_drawing_speed()
        self.verticalLayout_mouse_settings.addLayout(self.verticalLayout_drawing_speed)

        # add waiting speed
        self.UI_vertical_layout_waiting_speed()
        self.verticalLayout_mouse_settings.addLayout(self.verticalLayout_waiting_speed)
    ...
    def UI_grid_layout_1(self):
        ...
        self.gridLayout_1.addLayout(self.verticalLayout_output_size, 5, 2, 1, 1)

    def UI_img_menu_size(self):
        self.gridLayout_menu_img_size = QtWidgets.QGridLayout()

        self.doubleSpinBox_afbeelding_menu_grootte = QtWidgets.QDoubleSpinBox(self.centralwidget)
        ...
        self.doubleSpinBox_afbeelding_menu_grootte.setProperty("value", self.img_menu_size)
        self.doubleSpinBox_afbeelding_menu_grootte.valueChanged.connect(self.update_menu_img)
        # TODO relocate this
        self.gridLayout_menu_img_size.addWidget(self.doubleSpinBox_afbeelding_menu_grootte, 0, 0)

        self.label_afbeelding_menu_grootte = QtWidgets.QLabel(self.centralwidget)
        # TODO relocate this
        self.gridLayout_menu_img_size.addWidget(self.label_afbeelding_menu_grootte, 0, 1)
    ...
    def UI_vertical_layout_drawing_speed(self):
        ...
        self.horizontalSlider_drawing_speed = QtWidgets.QSlider(self.centralwidget)
        self.horizontalSlider_drawing_speed.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        self.horizontalSlider_drawing_speed.setMinimum(0)
        self.horizontalSlider_drawing_speed.setMaximum(len(self.AVAILABLE_DRAWING_SPEEDS) - 1)
        self.horizontalSlider_drawing_speed.setValue(self.DEFAULT_DRAWING_SPEED_IDX)
        self.horizontalSlider_drawing_speed.setPageStep(1)
        self.horizontalSlider_drawing_speed.setOrientation(QtCore.Qt.Horizontal)
        ...
        self.horizontalSlider_drawing_speed.setTickPosition(QtWidgets.QSlider.TicksBelow)
        self.horizontalSlider_drawing_speed.setTickInterval(1)
        self.horizontalSlider_drawing_speed.valueChanged.connect(self.update_drawing_speed)
        self.verticalLayout_drawing_speed.addWidget(self.horizontalSlider_drawing_speed)
    ...
    def UI_vertical_layout_waiting_speed(self):
        ...
        self.horizontalSlider_waiting_speed = QtWidgets.QSlider(self.centralwidget)
        self.horizontalSlider_waiting_speed.setMinimum(0)
        self.horizontalSlider_waiting_speed.setMaximum(len(self.AVAILABLE_WAITING_SPEEDS) - 1)
        self.horizontalSlider_waiting_speed.setValue(self.DEFAULT_WAITING_SPEED_IDX)
        self.horizontalSlider_waiting_speed.setPageStep(1)
        self.horizontalSlider_waiting_speed.setOrientation(QtCore.Qt.Horizontal)
        self.horizontalSlider_waiting_speed.setTickPosition(QtWidgets.QSlider.TicksBelow)
        self.horizontalSlider_waiting_speed.setTickInterval(1)
        self.horizontalSlider_waiting_speed.valueChanged.connect(self.update_waiting_speed)
        self.verticalLayout_waiting_speed.addWidget(self.horizontalSlider_waiting_speed)
    ...
    def retranslateUi(self, Image_drawer):
        ...
        Image_drawer.setWindowTitle(_translate("Image_drawer", "image-plotter"))
        self.pushButton_generate_output.setText(_translate("Image_drawer", "Generate output"))
        self.checkBox_display_image_placing.setText(_translate("Image_drawer", "Display preview"))
        ...
        self.label_output_resolution_x.setText(_translate("Image_drawer", "x (px)"))
        self.label_output_resolution_y.setText(_translate("Image_drawer", "y (px)"))
        self.label_drawing_speed_0.setText(_translate("Image_drawer", "Drawing speed"))
        self.label_drawing_speed_1.setText(_translate("Image_drawer", "Slow and steady"))
        self.label_drawing_speed_2.setText(_translate("Image_drawer", "I trust my computer"))
        self.label_drawing_speed_3.setText(_translate("Image_drawer", "Zoom"))
        self.label_waiting_speed_0.setText(_translate("Image_drawer", "Waiting speed"))
        ...
        self.label_color_pallete.setText(_translate("Image_drawer", "Color pallete preview:"))
        self.menuFiles.setTitle(_translate("Image_drawer", "Files"))
        self.actionSave_output.setText(_translate("Image_drawer", "Save output file"))
        self.actionLoad_input.setText(_translate("Image_drawer", "Load input file"))
    ...
    def init_thread(self):
        ...
        self.drawing_thread.progress_signal.connect(self.update_progressBar_main)
    ...
    def update_output_resolution(self):
        ...
        self.output_img_size = (
            value,
            int(self.output_img_size[1] / self.output_img_size[0] * value),
        )
        self.spinBox_output_resolution_y.setProperty("value", self.output_img_size[1])
    ...
    def resize_floating_placer(self):
        self.floating_image_preview_window.resize_window(int(self.dial_output_size.value()))

    def opacity_floating_placer(self):
        self.floating_image_preview_window.change_opacity(int(self.dial_output_opacity.value()))
    ...
    def open_file_dialog(self):
        ...
        self.input_img = ImgFile(self.input_fileName, self.img_menu_size)
    ...
    def load_color_from_cursor(self):
        if self.checkBox_input_colors.isChecked() or self.checkBox_set_background_color.isChecked():
            # Start Button action:
    ...
    def add_color_to_pallete(self, data: tuple):
        if self.checkBox_input_colors.isChecked() or self.checkBox_set_background_color.isChecked():
            ...
            if self.checkBox_set_background_color.isChecked():
                ...
                self.colorBox_background_color.update_color(self.background_color)
    ...
    def empty_palette_error(self):
        ...
        error_dialog.setInformativeText("There are no colors in the colorpalette")
        error_dialog.setWindowTitle("Error")
        error_dialog.exec_()
    ...
    def process_floyd(self, outfile_name: str):
        del self.output_img
        self.output_img = ImgFile(outfile_name, self.img_menu_size)
        self.update_menu_img()
    ...
    def draw_output(self):
        ...
        drawing_domain = (win_x + img_x, win_y + img_y, win_x + img_x2, win_y + img_y2)
        ...
        self.drawing_thread.start_drawing(
            drawing_domain,
            self.color_pallette,
            self.output_img,
            self.home,
...
if __name__ == "__main__":
    ...
    icon = QtGui.QIcon()
    icon.addPixmap(QtGui.QPixmap("./sources/eend.png"), QtGui.QIcon.Selected, QtGui.QIcon.On)
    # self.setWindowIcon(icon)
